...
- [x] **Task 1 — `scripts/utils/views.py`: генерация DDL типизированных view'ов из каталога (AC: #1, #2, #3, #4, #5, #6, #7, #8)**
...
- [x] **Task 3 — Offline-тесты `tests/test_views.py` (AC: #1–#8)**
  ...
  - [x] **AC #1 + #3 + #5 (интеграция, типы):** в `tmp_path` положить мини-партиции `data/raw/visits/2026-05-20.parquet` и `data/raw/hits/…` со snake_case-колонками **VARCHAR** (значения строками as-is, как пишет 2.2 — можно записать прямой `duckdb …write_parquet`, чтобы не импортировать `parquet_store` в модуль; в тесте `write_partition` использовать допустимо как хелпер). `create_views(conn)` → `SELECT visit_id, watch_ids, date FROM visits` отдаёт **HUGEINT/`LIST`/DATE** (проверить `typeof`/значения); `visit_id` за пределами BIGINT (напр. `17298374650000000001`) не переполняется (HUGEINT, NFR-4); родное `ym:s:*` в запросе не используется.
  - [x] **AC #2 (битая ячейка → NULL, view не падает):** партиция с битым значением в типизируемой колонке (напр. `page_views='abc'` для INTEGER, `date='not-a-date'`) → `SELECT` проходит, битая ячейка = `NULL`, соседние валидные строки целы (view не падает).
  - [x] **AC #8 (массив → `LIST`):** `watch_ids='[8273645,8273646]'` → `len(watch_ids)=2`, элементы HUGEINT; пустой `watch_ids='[]'` → пустой `LIST` (**не `[NULL]`** — закрепляет преимущество варианта A над B); битый `watch_ids='garbage'` → `NULL`. _(При выборе варианта B Шефом — переписать ожидания под поэлементный разбор + гард `[]`.)_
  - [x] **AC #4 (перезалив отражается без материализации):** записать партицию дня → `create_views` → запрос даёт N строк; **перезаписать** ту же партицию (другое содержимое/число строк, приём 2.2 `os.replace`) → **тот же** view (без пере-создания) отражает новые данные. Доказывает ленивость view.
  - [x] **AC #7 (дрейф схемы, `union_by_name`):** две партиции visits с **разным набором колонок** (старая без нового поля, новая с ним) → запрос нового поля даёт `NULL` для строк старой партиции, реальное значение для новой; обе партиции читаются (нет ошибки позиционного матча).
  - [x] **AC #6 (пустой источник):** (a) каталог источника без `.parquet` → `create_views` создаёт **пустой типизированный** view (`SELECT count(*)=0`, но `SELECT … LIMIT 0` даёт правильные типы колонок); (b) **visits есть, hits пуст** → `visits` отдаёт строки, `hits` — пустой типизированный view; пустой источник **не валит** построение другого (граница FR-3). Дополнительно: осиротевший `{date}.parquet.tmp` в каталоге источника **не** считается партицией (источник с одним `.tmp` → пустой view).
  - [x] **Битый корень (наследование fail-loud):** `monkeypatch.delenv(DATA_ROOT_ENV)` + `create_views(conn)` → `ValueError` (из `get_storage_root`/`get_raw_source_dir`) — наследуется из `paths` (2.1), без побочных эффектов.
  - [x] **Анти-зависимость (через `ast`, по реальным import-узлам — приём `test_parquet_store.py:387`):** в `scripts/utils/views.py` нет top-level import `pandas`/`polars`/`numpy`/`pyarrow`, directaiq-инфры `config_manager`/`base_script`; **дополнительно (риск №6):** нет импорта `scripts.utils.database_manager`/`scripts.utils.parquet_store`/`scripts.utils.load_state`/`scripts.utils.writer_lock`. `duckdb` **разрешён** (как 2.4 — нужен для аннотации `conn`).
  - [x] **Live-тест НЕ нужен** (и не заводить): `views.py` в сеть не ходит — генерирует DDL и читает локальный Parquet. Правило opt-in live ([[realapi-smoke-tests]]) — только для внешнего Logs API. Зафиксировать в Dev Agent Record (как 2.1/2.2/2.3/2.4/2.5). **Отдельно отметить:** окончательное подтверждение формата TSV-массива (особенно экранирование строковых массивов) — за live-smoke оркестратора 2.7; фикстуры освежить из реального ответа.
- [x] **Task 4 — Гейты верификации (обязательны перед закрытием)**
  - [x] `uv run mypy scripts` → зелено (strict; `conn: duckdb.DuckDBPyConnection`; `Iterable[str]`; без `Any`-дыр).
  - [x] `uv run pytest` → зелено (новый offline-набор + регрессия 1.x/2.1–2.5; live отсеян `addopts="-m 'not live'"`). На обеих ОС (glob/`union_by_name`/`as_posix`).
  - [x] Новых зависимостей нет (`duckdb` уже в стеке; `logging`/`re`/`collections.abc` — stdlib) → **`uv.lock` не меняется**.
...
- `uv run pytest tests/test_views.py -q` → 15 passed (новый набор AC #1–#8 + чистый билдер + битый корень + ast-анти-зависимость).
- `uv run mypy scripts` → `Success: no issues found in 18 source files` (strict; `conn: duckdb.DuckDBPyConnection`, `Iterable[str]`, без `Any`-дыр).
- `uv run mypy scripts --platform linux` → `Success` (кросс-платформенно; `views.py` платформо-независим — нет `fcntl`/`msvcrt`).
- `uv run pytest -q` → 267 passed, 3 deselected (live отсеян `addopts="-m 'not live'"`; регрессия 1.x/2.1–2.5 цела, +15 новых).
- `git diff --stat uv.lock` → пусто (новых зависимостей нет: `duckdb` уже в стеке, `logging`/`collections.abc` — stdlib).
- `git status --short` → только `scripts/utils/views.py` + `tests/test_views.py` (артефактов `data/`/`*.parquet`/`*.duckdb`/`.writer.lock` в dev-репо не создано).

### Completion Notes List

- **Реализован вариант A (нативный `TRY_CAST(col AS T[])` для массивов)** — утверждён Шефом 2026-05-24. Билдер DDL единообразен: массив и скаляр типизируются одним выражением `TRY_CAST("{storage}" AS {duckdb_type})`, `duckdb_type` уже несёт `T[]` для массивов (из каталога). Тест `test_views_array_typed_as_list` подтверждает исход AC #8: `[8273645,8273646]`→`LIST` (len 2), пустой `[]`→`[]` (len 0, **не** `[None]`), `garbage`→`NULL`.
- **`scripts/utils/views.py`:** чистый `build_view_ddl(source, catalog, *, partition_glob, has_partitions) -> str` (тестируем без БД) + тонкий исполнитель `create_views(conn, *, catalog=None, sources=VALID_SOURCES) -> None`. `conn` инъектируется (БД сам не открывает); `catalog=None`→`load_catalog()`. **НЕ** импортирует `database_manager`/`parquet_store`/`load_state`/`writer_lock` (риск №6, закреплено ast-тестом); `duckdb` разрешён (аннотация `conn`, как 2.4).
- **Риски закрыты:** №2 — путь в DDL строковым литералом, экранирование `'`→`''` + `as_posix()` (тест `..._escapes_single_quote_in_path`); №3 — пустой источник → `CAST(NULL AS type)` + `WHERE false` без `read_parquet`, `.parquet.tmp` не считается партицией (тесты `empty_source`/`orphan_tmp`); №4 — `read_parquet(..., union_by_name => true)`, дрейф схемы → отсутствующая колонка `NULL` (тест `schema_drift_union_by_name`); №5 — `TRY_CAST` даёт NULL без per-cell лога (зафиксировано в docstring, AC #2 по контракту «битая→NULL, день не падает»); №7 — `CREATE OR REPLACE VIEW` + ленивый view → перезалив виден без материализации (тест `..._reflects_repartition_without_recreate`); №8 — квотированные snake_case-имена, `ym:s:*` во view нет (assert `"ym:s:" not in ddl`); №9 — конкуренция читатель↔писатель (Windows `os.replace`) зафиксирована как рантайм-забота MCP 3.1, не 2.6 (docstring).
- **Транзиентная граница (НЕ в скоупе AC #7):** поле в каталоге, но НИ ОДНОЙ партиции с ним → `TRY_CAST("new_col")` упадёт «column not found» при запросе. Окно ничтожно (p81 2.7 выгружает полный список полей каждый день) — зафиксировано в docstring, не усложняем `COALESCE`'ом. Тест дрейфа использует мини-каталог, где каждое поле есть хотя бы в одной партиции.
- **Доп. строгий гард:** источник без полей в каталоге → `ValueError` (вырожденный SSOT = дефект; тест `..._rejects_source_without_fields`). Поэтому тесты с одно-источниковым каталогом передают `sources=(...)` явно.
- **`docs/working-layer.md` дополнен** разделом «Типизированные представления» (3 вопроса простыми словами + границы 2.2/2.4/2.7/3.1; явно «представления — только чтение, замок не берут»); вводная заметка и секция «Границы» обновлены под наличие `views.py`.
- **Live-набор осознанно отсутствует** (как 2.1–2.5): `views.py` в сеть не ходит — генерирует DDL и читает локальный Parquet. Окончательное подтверждение формата TSV-массива (особенно экранирование строковых массивов с запятой/скобкой внутри) — за live-smoke оркестратора 2.7; offline-фикстуры освежить из реального ответа ([[realapi-smoke-tests]], defer 2.3).

### File List

- `scripts/utils/views.py` — новый: рабочий слой, билдер DDL + исполнитель view'ов (Task 1).
- `tests/test_views.py` — новый: offline-тесты AC #1–#8 + битый корень + ast-анти-зависимость (Task 3).
- `docs/working-layer.md` — изменён: добавлен раздел «Типизированные представления», обновлены вводная заметка и «Границы» (Task 2).
...
- 2026-05-25 — Story 2.6 реализована (dev-story): `scripts/utils/views.py` — чистый `build_view_ddl` + исполнитель `create_views(conn)` (вариант A — нативный `TRY_CAST(col AS T[])`); `conn` инъектируется, БД сам не открывает, лок/`mkdir`/сеть не трогает, `database_manager`/`parquet_store`/`load_state`/`writer_lock` не импортирует (риск №6). Закрыты AC #1–#8 и риски №2–№9. Дополнен `docs/working-layer.md` (раздел «Типизированные представления», 3 вопроса + границы 2.2/2.4/2.7/3.1, «представления — только чтение»). Offline-набор `tests/test_views.py` (15 тестов: чистый DDL без БД + интеграция на tmp-партициях + битый корень + ast-анти-зависимость, `duckdb` разрешён); live осознанно отсутствует (формат TSV-массива подтверждает live-smoke 2.7). Гейты зелёные: `mypy strict` 18 файлов (win32 + `--platform linux`), `pytest` 267 passed / 3 deselected; `uv.lock` не менялся; артефактов данных в dev-репо не создано. Статус → review.

## Review Findings (Code Review, 2026-05-25)

Три адверсариальных слоя (Blind Hunter / Edge Case Hunter / Acceptance Auditor, все Opus). Acceptance Auditor: все 8 AC — PASS, инварианты (snake_case, HUGEINT для UInt64, CAST только во view, `ym:s:*` только в каталоге) и риски №2/№6/№9 соблюдены; Dev Agent Record сверен с фактом (15/267 passed, mypy 18 файлов, `uv.lock` чист) — расхождений нет. Ниже — находки слоёв корректности/edge-кейсов: 1 решение, 3 патча, 11 отброшено как шум.

- [x] [Review][Patch] Glob-метасимволы в `GDAU_DATA_ROOT` ломают резолюцию партиций — путь уходит в `read_parquet('{glob}')` строковым литералом; экранируется только одинарная кавычка, glob-классы `[ ] ? *` — нет. Корень вида `…/game[1]/…` → DuckDB истолкует `[1]` как класс символов → реальные `.parquet` не находятся (воспроизведено Edge Case Hunter): view молча 0 строк или «No files found» при запросе. **Решение Шефа (2026-05-25): вариант B — экранировать glob-метасимволы** (`glob.escape(str(source_dir))` перед добавлением шаблона `*.parquet`); форма DDL (glob) и контракт `build_view_ddl` сохраняются. Затрагивает только DuckDB-литерал `read_parquet` (Python-side `source_dir.glob` якорит каталог литерально — pathlib не глоббит базовый путь). [scripts/utils/views.py:150]
- [x] [Review][Patch] Каталог с именем `*.parquet` ошибочно считается партицией — `glob("*.parquet")` матчит и директории; `has_partitions=True` → DDL по ветке `read_parquet`, но при запросе IOException (расходится с веткой пустого источника). Фикс: фильтр `is_file()` (как в `load_state._partition_dates`). [scripts/utils/views.py:145]
- [x] [Review][Patch] Имя view не квотируется в DDL вопреки собственной конвенции (риск №8: «квотируй идентификаторы двойными кавычками») — колонки квотированы, а `CREATE OR REPLACE VIEW {source}` — нет. Фикс: `VIEW "{source}"`. [scripts/utils/views.py:97,110]
- [x] [Review][Patch] Неатомарное создание набора view'ов при падении на втором источнике — если `build_view_ddl` бросит `ValueError` на `hits` после `conn.execute` для `visits`, получится частично определённый слой. Фикс: собрать все DDL до первого `conn.execute`. [scripts/utils/views.py:140-157]

- 2026-05-25 — Code review (3 слоя, Opus): все 8 AC — PASS (Acceptance Auditor), инварианты/риски №2/№6/№9 соблюдены. 4 патча применены + покрыты регресс-тестами: (1) `is_file()`-фильтр — каталог с именем `*.parquet` не партиция; (2) квотирование имени view `"{source}"` в обеих ветках (риск №8); (3) атомарность — все DDL собираются до первого `conn.execute`; (4) `glob.escape` каталога-литерала против glob-метасимволов корня (`…/game[1]/…`, вариант B по решению Шефа). 1 decision разрулён в патч, 0 defer, 11 dismiss. Гейты зелёные: `mypy strict` 18 файлов (win32 + linux), `pytest` 270 passed / 3 deselected (+3 теста); `uv.lock` не менялся. Статус → done.
