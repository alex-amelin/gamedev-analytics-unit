...
пространства (``CLAUDE.md``), файл описания игры (``gdd.md``) и каталог событий
аналитики (``EVENTS.md``). Этот модуль — ПРИМИТИВ копирования шаблона в папку игры:
валидирует наличие шаблона fail-loud ДО любых мутаций (битый шаблон не родит полу-пустое
хранилище, AC #5), копирует файлы и НЕ затирает уже заполненные владельцем ``gdd.md`` и
``EVENTS.md`` при повторном развороте (AC #6).
...
бережёт ``gdd.md``/``EVENTS.md``. Модуль в сеть не ходит, ``gdau.duckdb`` не открывает, данные/
...
# gdd.md (описание игры) и EVENTS.md (каталог событий) заполняет владелец — затереть их
# шаблоном = потерять внесённый контекст игры (AC #6).
PRESERVE_ON_REPEAT = ("gdd.md", "EVENTS.md")
...
def copy_storage_template(
    *, storage_root: Path, template_root: Path | None = None
    ...
    ``gdd.md``/``EVENTS.md``), **пропускается** — не затирается (AC #6).

    Возвращает список фактически скопированных путей (для лога/диагностики 4.3; пропущенные
    ``gdd.md``/``EVENTS.md`` в него не входят). Частичную копию НЕ откатывает — полный откат хранилища
    ...
    try:
        ...
        for entry in sorted(root.iterdir()):
            ...
            # Сохранение заполненных владельцем gdd.md/EVENTS.md при повторном init (AC #6).
            if entry.name in PRESERVE_ON_REPEAT and dest.exists():
                logger.info(
                    "Сохраняю существующий %s (повторный init — не затираю)", entry.name
                )
                continue
            # Шаблон сейчас плоский (5 файлов), но код терпим к будущим подкаталогам.
            if entry.is_dir():
