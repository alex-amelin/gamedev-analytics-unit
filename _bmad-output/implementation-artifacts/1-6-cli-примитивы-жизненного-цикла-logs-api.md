...
- [x] **Task 1 — Каркас модуля + `_create_parser` со всеми подкомандами (AC: #1, #6)**
  - [x] Заменить стаб `scripts/tools/logs_api_cli.py` (история 1.1: `print("not yet implemented")`) реальным CLI. `from __future__ import annotations` первой строкой. Импорты — абсолютные от корня пакета: `from scripts.utils.metrica_client import MetricaClient`, `from scripts.utils.env_reader import read_metrica_credentials`, `from scripts.utils.catalog import load_catalog`, `from scripts.utils.dates import parse_date, format_date, clamp_date_range`. Stdlib: `argparse`, `logging`, `sys`, `from pathlib import Path` (форматтеры `csv`/`json`/`io` НЕ нужны — вывод человекочитаемый). **Без** `BaseScript`/`AuthManager`/`get_logger`/`setup_paths`/`config_manager`/`polars`/`pandas` — инфру и тяжёлые зависимости directaiq не тащим (NFR-6, [[directaiq-reference]]).
  - [x] Русский модульный docstring: роль (AI-native канал действий поверх Logs API; склейка 1.2–1.5; примитивы FR-1, применение FR-2; `update` — это 2.9, не здесь). `logger = logging.getLogger(__name__)`.
  - [x] Класс `LogsApiCLI` с `_create_parser(self) -> argparse.ArgumentParser` (форма directaiq). `description` + `formatter_class=argparse.RawDescriptionHelpFormatter`. **Без** глобального `--format` (решение Шефа: вывод человекочитаемый, как directaiq — см. риск #4 и Task 2).
  - [x] `subparsers = parser.add_subparsers(dest="command", required=True, ...)` — **`required=True`** закрывает AC #6 (голый вызов → argparse usage + exit 2, без трейсбека).
  - [x] Подкоманды жизненного цикла (без `--fields` у create/evaluate — риск #1; даты строками, валидирует `dates.py`):
    ...
    - `list`: без аргументов.
  - [x] Info-подкоманда `info` (→ `get_counter_info`) — проверка доступа/счётчика. Без аргументов. _Решение Шефа: только `info`._ **Обоснование состава:** architecture (строка 460) пишет `info` в ед. числе; в directaiq-`logs_api_cli` info-команд НЕ было вовсе (счётчик/цели жили в отдельных `metrica_cli.py`/`goals_cli.py`). `counters`/`goals` НЕ выставляем: `counter_id` приходит из `.env` (единый источник кредов — discovery не нужен), а `goals` — маркетинг-семантика Директа, нерелевантная сессиям/хитам геймдева (NFR-6, простота). epic AC #1 «info-подкоманды» удовлетворяется единственной `info` плюс уже перечисленными lifecycle-подкомандами.
- [x] **Task 2 — Общие швы: креды+клиент, поля из каталога, человекочитаемая печать (AC: #2, #4, #5)**
  - [x] `_build_client(self) -> MetricaClient` — `creds = read_metrica_credentials(); return MetricaClient(token=creds.token, counter_id=creds.counter_id)`. Единственная точка построения клиента (риск #3). `ValueError` отсюда (нет кредов) **не** ловить здесь — всплывает в `main()` (AC #4). Info/lifecycle-команды зовут его; клиент строить **после** разбора аргументов и валидации дат, не в парсере.
  - [x] Поля выгрузки: `fields = load_catalog().metrica_fields(source)` — **в `create`/`evaluate`** (риск #1, FR-2). `load_catalog()` без аргумента (прод-путь, резолвится от модуля сквозь симлинк — см. 1.5). Битый каталог → `ValueError` всплывает в `main()` (non-zero).
  - [x] **Печать — человекочитаемая, как directaiq (AC #5; без `--format`).** Каждый `_handle_*` сам печатает результат через `print()` чистым текстом (диспетчер централизованно НЕ форматирует). stdout = полезный вывод; диагностика (`logger.info` про clamp/ретраи) идёт в stderr через `logging` — не смешивать. Форма (как directaiq):
  ...
  - [x] Handler'ы дополнительно **возвращают** структурный результат (dict/list) — это для тестов (capsys проверяет печать, возврат — точечные assert'ы) и возможного переиспользования; `main()` сам ничего не печатает.
- [x] **Task 3 — Handler `create` (AC: #2, #8)**
  - [x] `_handle_create(self, args) -> dict`: `d1 = parse_date(args.date1); d2 = parse_date(args.date2)` → `d1c, d2c = clamp_date_range(d1, d2)` → `fields = load_catalog().metrica_fields(args.source)` → `client = self._build_client()` → `resp = client.create_log_request(date1=format_date(d1c), date2=format_date(d2c), fields=fields, source=args.source, attribution=args.attribution)`.
  - [x] Извлечь `log_request = resp.get("log_request", resp)` (риск #5 — `create_log_request` отдаёт полный ответ; нужное под `log_request`). **Напечатать** человекочитаемо: `request_id` и `status` (AC #2 «возврат id/статуса»; AC #5 — текст), + подсказку «проверь статус командой `status`». Вернуть `log_request` (для тестов).
  - [x] Отказ API (квота/невозможно) → `MetricaClient` бросает `RuntimeError` (1.3: не-ретраябельные коды и тело ошибки → понятный `RuntimeError`) → всплывает в `main()` → non-zero + сообщение (AC #8, часть «обречённый запрос»). CLI **не** реализует retry/квоту заново (NFR-3, [[directaiq-reference]]).
- [x] **Task 4 — Handler `evaluate` (AC: #3, #8)**
  - [x] `_handle_evaluate(self, args) -> dict`: те же `parse_date`/`clamp_date_range`/`format_date` и `fields` из каталога (как create — чтобы оценка совпадала с тем, что create реально отправит). `resp = client.evaluate_log_request(date1=..., date2=..., fields=fields, source=args.source)`.
  - [x] Извлечь `ev = resp.get("log_request_evaluation", resp)` (риск #5). **Напечатать** `possible` и `max_possible_day_quantity` человекочитаемо (AC #5). Несёт смысл для пред-проверки перед `create` (AC #8 «possible=false по evaluate» — `evaluate` это и выставляет; авто-вызова evaluate из create НЕ делаем, простота). Вернуть `ev`.
- [x] **Task 5 — Handlers `status` / `list` (AC: #3, #5, #7)**
  - [x] `_handle_list(self, args) -> list[dict]`: `reqs = client.get_log_requests()` (уже `list`, риск #5). **Напечатать** выровненной таблицей (`request_id`/`status`/`source`/диапазон/размер — как `_handle_list` directaiq); пустой список → строка «нет активных запросов», результат `[]` (валидно, не ошибка). Вернуть `reqs`.
  - [x] `_handle_status(self, args) -> dict`: `data = client.get_log_request(args.request_id)` (уже извлечённый внутренний dict, риск #5). **Несуществующий `request_id`:** клиент на 404 бросит `RuntimeError` (1.3, не-ретраябельный) → всплывает (AC #7); но если API вернёт 200 с пустым `log_request` → `data == {}` → **fail-loud**: `if not data: raise ValueError(f"Запрос {request_id} не найден")` (AC #7 — пустой ответ НЕ выдавать за успех). _Не повторять directaiq, где пустой `data` логировался и возвращался `{}` как «успех»._ Иначе **напечатать** `request_id`/`status`/диапазон/число частей + размер каждой части (AC #5). Вернуть `data`.
- [x] **Task 6 — Handler `download`: статус-гейт, выбор частей, no-clobber, без ложного успеха (AC: #3, #7, #8)**
  - [x] `_handle_download(self, args) -> dict`. **Сначала** `info = client.get_log_request(args.request_id)`; `if not info: raise ValueError("запрос не найден")` (AC #7); `if info.get("status") != "processed": raise ValueError(f"статус '{info.get('status')}', скачивание невозможно (нужен 'processed')")` — non-zero, **ничего не пишем** (AC #7, «неполная/пустая выгрузка не = успех»). _directaiq логировал и возвращал info как «успех» — у нас это fail._
  - [x] `parts = info.get("parts", [])`; пусто → `ValueError` (нечего качать, не «успех»). `--part` указана, но её нет в `parts` → `ValueError` (AC #7).
  - [x] Резолюция вывода (БЕЗ `paths.py`/storage — их нет до 2.1; в dev-репо данные не пишем): `--output` задан → если суффикс есть, трактовать как файл (каталог = `parent`, префикс = `stem`), иначе как каталог; не задан → **текущий каталог запуска** (`Path.cwd()`) с префиксом `logs_{request_id}`. `output_dir.mkdir(parents=True, exist_ok=True)`. Имя части: `{prefix}_part{n}.tsv`.
  - [x] **No-clobber (AC #8):** перед записью каждой части `if filepath.exists(): raise FileExistsError(f"{filepath} уже существует — укажи другой --output (без молчаливой перезаписи)")`. _directaiq делал `open(..., "wb")` — молчаливая перезатирка; у нас запрещено._
  - [x] Качаем части: `content = client.download_log_request_part(request_id, part_number)` (`bytes`, риск #5) → `filepath.write_bytes(content)`. Любой сбой части (исключение клиента) всплывает → non-zero, **не** «собрали что есть» (AC #7).
  - [x] `--clean` → после успеха всех частей `client.clean_log_request(request_id)`.
  - [x] **Напечатать** сводку человекочитаемо (какие файлы сохранены, сколько частей, очищен ли запрос) и вернуть `{"downloaded": [str(p) for p in files], "parts": len(files), "cleaned": bool(args.clean)}` (для тестов).
- [x] **Task 7 — Handler `clean` (AC: #3)**
  - [x] `_handle_clean(self, args) -> dict`: `resp = client.clean_log_request(args.request_id)`; извлечь `lr = resp.get("log_request", resp)` (риск #5). **Напечатать** новый `status` человекочитаемо. Вернуть `lr`.
- [x] **Task 8 — Info-подкоманда `info` (AC: #1, #3, #5)**
  - [x] `info` → `data = client.get_counter_info()` (dict). Строит клиент через `_build_client` (нужны креды/сеть). **Напечатать** ключевые поля счётчика человекочитаемо (AC #5); вернуть `data`. `counters`/`goals` НЕ заводим (обоснование — Task 1).
- [x] **Task 9 — `main()`: диспетчер + коды возврата + логирование (AC: #4, #6)**
  - [x] `_dispatch(self, args) -> object`: по `args.command` вызвать соответствующий `_handle_*` (handler сам печатает результат — Task 2); неизвестная команда невозможна (`required=True` + `choices`), но defensive `else: raise ValueError`.
  - [x] `def main() -> None`: `logging.basicConfig(level=logging.INFO)` (диагностика clamp/ретраев в stderr; **креды не логируем** — NFR-5). `cli = LogsApiCLI(); parser = cli._create_parser(); args = parser.parse_args()` (плохие аргументы/голый вызов → argparse сам `SystemExit(2)`, AC #6). Затем:
    ```python
    try:
    ...
    ```
  - [x] `if __name__ == "__main__": main()`. Entry point `gdau-logs = scripts.tools.logs_api_cli:main` уже в `pyproject.toml` (1.1) — не трогать.
  - [x] Перехватывать **узкий** набор исключений (`ValueError`/`RuntimeError`/`FileExistsError`/`OSError`) — это контрактные ошибки 1.2–1.5 и клиента; не глотать `KeyboardInterrupt`/`SystemExit`/прочее «голым» `except` (иначе argparse `SystemExit(2)` исказится).
- [x] **Task 10 — Спека компонента `docs/cli.md` (часть DoD)**
  - [x] Завести `docs/cli.md` человеческим языком (project-context прямо называет `cli.md` отдельным логическим компонентом: «поверхность `gdau-logs`»). Три вопроса: **(1) Что делает** — выставляет жизненный цикл Logs API подкомандами (заказать/оценить/статус/скачать/очистить/список + справочные), неинтерактивно; **(2) Зачем** — AI-native канал действий: агент собирает ad-hoc обращения к API из примитивов-команд ([[cli-tools-ai-native]]); **(3) Контракт** — вход: подкоманда + аргументы; поля выгрузки и типы — из каталога (не руками); даты клампятся на «вчера по МСК»; креды из `.env`; выход: результат **человекочитаемым текстом** (просто и понятно агенту и оператору), успех → код 0, любой сбой → понятное сообщение + non-zero. Упомянуть границу: **`update`/полный приём — отдельная команда (Epic 2)**, здесь только примитивы. Без сигнатур кода.
- [x] **Task 11 — Offline-тесты `tests/test_logs_api_cli.py` (AC: #1–#8)**
  - [x] `from __future__ import annotations`; без сети. Зеркалит `scripts/` → `tests/test_logs_api_cli.py`. Сеть мокается **monkeypatch'ем имён в неймспейсе CLI-модуля** (`MetricaClient`, `read_metrica_credentials`, `load_catalog`) — фейковый клиент с заранее заданными ответами; **никакого requests-mock** (нет такой dev-зависимости — только `pytest`/`mypy`; см. `pyproject`). Шов: фейк-класс с методами `create_log_request`/`get_log_request`/… возвращает фикстурные dict/bytes.
  - [x] **AC #1:** `_create_parser().parse_args(["--help"])` → `SystemExit(0)`; `capsys` содержит `create`, `status`, `download`, `clean`, `evaluate`, `list`, `info`.
  - [x] **AC #2 (create):** monkeypatch `load_catalog` → фейк с `metrica_fields("visits")==["ym:s:date","ym:s:visitID"]`; monkeypatch `read_metrica_credentials`/`MetricaClient` → фейк, фиксирующий аргументы `create_log_request`. Прогон `main()` с `create --date1 ... --date2 <будущее> --source visits` → проверить: (a) клиент получил `fields` ровно из каталога (FR-2, **не** из CLI); (b) `date2` зажат на «вчера по МСК» (clamp 1.4 — передать фикс «сегодня»? clamp берёт `moscow_today()` внутри create-пути → проверять, что `date2` ≤ вчера, через monkeypatch `dates.moscow_today` ИЛИ через то, что фейк-клиент получил клампнутую строку < сегодня); (c) токен/счётчик пришли от `read_metrica_credentials`; (d) exit 0, вывод несёт `request_id`/`status`.
  - [x] **AC #3 (прокси):** для `status`/`list`/`clean`/`evaluate`/`info` — фейк-клиент с известным ответом → проверить, что вызван правильный метод с правильными аргументами и результат напечатан (`capsys`).
  - [x] **AC #4 (коды/сообщения):** нет кредов → monkeypatch `read_metrica_credentials` бросает `ValueError("YANDEX_METRICA_TOKEN ...")` → `main()` → `SystemExit` с кодом 1, stderr несёт сообщение, **токен в выводе не светится**; ошибка API → фейк-клиент бросает `RuntimeError` → exit 1 + сообщение. Успех → exit 0.
  - [x] **AC #5 (человекочитаемый вывод, без `--format`):** `capsys` после `status`/`create`/`list` содержит ключевые поля понятным текстом (напр. `Request ID`/`Status`, значения `request_id`/`status`); `list` печатает строку на каждый запрос; пустой `list` → строка «нет активных запросов». Проверить, что парсер **не** принимает `--format` (`parse_args(["--format","json","list"])` → `SystemExit(2)`).
  - [x] **AC #6 (argparse-гард):** `parse_args([])` (голый, без подкоманды) → `SystemExit` код 2; `create --date1 .. --date2 .. --source sessions` (вне choices) → `SystemExit` код 2; оба — `capsys` содержит usage, без трейсбека.
  - [x] **AC #7 (ранний download/status):** фейк `get_log_request` → `{"status":"created","parts":[]}` → `download` поднимает ошибку, **ни один файл не записан** (проверить `tmp_path` пуст), exit 1; `get_log_request` → `{}` (не найден) → `status` и `download` → ошибка not-found, exit 1; `--part 5`, которой нет в `parts` → ошибка.
  - [x] **AC #8 (квота/клоббер):** фейк `create_log_request` бросает `RuntimeError("quota")` → exit 1 + сообщение; `download` в `tmp_path`, где целевой `logs_{id}_part1.tsv` **уже существует** → `FileExistsError` всплывает → exit 1, существующий файл **не перезаписан** (сверить содержимое до/после).
  - [x] **AC #2 невалидная дата:** `create --date1 2026-13-99 ...` → `parse_date` `ValueError` → exit 1 + сообщение, **до** построения клиента (фейк-клиент не вызван); инвертированный диапазон (`date1 > date2` после clamp) → `clamp_date_range` `ValueError` → exit 1.
  - [x] **Анти-зависимость (через `ast`, не подстроку):** распарсить `ast` модуля `logs_api_cli.py`, проверить отсутствие `Import`/`ImportFrom` на `pandas`, `polars`, `numpy`, и на directaiq-инфру `base_script`/`auth_manager`/`config_manager` (top-level имя `name.split(".")[0]`; приём из `test_catalog.py`/`test_env_reader.py`). Гарантирует «та же форма, но не та же обвязка» (NFR-6).
- [x] **Task 12 — Live-smoke `tests/test_logs_api_cli_live.py` (opt-in, обязателен для внешнего API)**
  - [x] По project-context и [[realapi-smoke-tests]]: компонент дёргает **реальный** Logs API → нужен opt-in live (моки не отражают контракт). `@pytest.mark.live`; нет кредов → `pytest.skip` с причиной (как `test_metrica_client_live.py`).
  - [x] **Минимальный и дешёвый — через `evaluate`** (GET оценки; **не жжёт** create/download-квоту, уважает ≤5000/day): построить `evaluate`-путь CLI на реальных кредах с **полным набором полей из реального каталога** за окно в 1 день. Цель — подтвердить **реальный контракт**: каталожные `metrica_fields(source)` (115 полей) **приняты** настоящим API (`possible` присутствует в ответе), т.е. список полей каталога не разошёлся с Logs API. Это уникальная ценность 1.6 поверх live-теста 1.3 (тот бьёт только `get_counter_info`).
  - [x] Прогонять для обоих источников visits и hits (по одному дешёвому `evaluate` на источник). Узкое окно (`date2`=вчера по МСК, `date1`=вчера) — минимум нагрузки.
- [x] **Task 13 — Гейты верификации (обязательны перед закрытием)**
  - [x] `uv run mypy scripts` → зелено (strict; CLI полностью типизирован; argparse `Namespace`-атрибуты — `Any`, но сигнатуры handler'ов аннотированы). Новых зависимостей нет → `uv.lock` не меняется.
  - [x] `uv run pytest` → зелено (новый offline-набор + регрессия 1.1–1.5; live отсеян `addopts="-m 'not live'"`).
  - [x] (Ручной, документировать) `uv run pytest -m live` против реального счётчика — проверить, что каталожные поля приняты API (см. Task 12). В CI не гоняется.
...
- `uv run pytest` → 156 passed, 3 deselected (live отсеян `addopts="-m 'not live'"`).
- `uv run pytest -m live` (без кредов в dev-репо) → 3 skipped — graceful skip, не ложный красный.
- Один промежуточный фейл был в самом тесте (`test_list_proxies_and_prints_table`: неверная
  проверка `" 1 " in ...` — request_id печатается в начале строки без ведущего пробела при
  left-justify); исправлено на устойчивую проверку «первый токен строки данных == request_id».
  Код CLI не затрагивался.

### Completion Notes List

- Стаб 1.1 (`print("not yet implemented")`) заменён реальным argparse-CLI `LogsApiCLI`
  (форма directaiq: `_create_parser` + per-command `_handle_*`), но БЕЗ инфры directaiq
  (`BaseScript`/`AuthManager`/`get_logger`/`setup_paths`/`config_manager`) — NFR-6.
- **Применение FR-2 (AC #2):** `create`/`evaluate` берут поля из `load_catalog().metrica_fields(source)`,
  флага `--fields` нет. **clamp дат (1.4):** `parse_date`→`clamp_date_range`→`format_date`, fail-loud
  на невалидной/инвертированной дате ДО сети. **Креды (1.2→1.3):** `read_metrica_credentials()` →
  `MetricaClient(token=, counter_id=)`, флага `--counter-id` нет.
- **Асимметрия ответов клиента (риск #5)** учтена: `create`/`clean` — `resp.get("log_request", resp)`;
  `evaluate` — `resp.get("log_request_evaluation", resp)`; `get_log_request` — прямой dict;
  `get_log_requests` — список; `download_log_request_part` — bytes.
- **Edge-cases:** `subparsers(required=True)` + `choices` → голый вызов/невалидный source = exit 2
  без трейсбека (AC #6); `download` со статус-гейтом `processed`, not-found/empty → fail-loud (AC #7);
  **no-clobber** с pre-flight проверкой существования ВСЕХ целевых файлов до любой записи/сети (AC #8);
  отказ create (квота) → `RuntimeError` всплывает → exit 1 (AC #8).
- **Вывод человекочитаемый** (метки `Поле: значение` + выровненная таблица для `list`), **без** `--format`
  (решение Шефа 2026-05-24, изменение epic AC #5); диагностика через `logging` в stderr, креды не логируются (NFR-5).
- `info` — единственная info-подкоманда (`get_counter_info`); `counters`/`goals` не заводятся (обоснование в Task 1).
- Спека `docs/cli.md` заведена (3 вопроса простыми словами) — DoD компонента.
- Тесты: 26 offline (AC #1–#8, вкл. невалидные даты/clamp/клоббер/ранний download/нет `--format`/
  анти-зависимость по `ast`); live-smoke `evaluate` подтверждает приём 115 каталожных полей реальным API (opt-in).
- Гейты: mypy strict ✅, pytest 156 ✅, `uv.lock` не менялся (stdlib-only, новых зависимостей нет).

### File List

- `scripts/tools/logs_api_cli.py` — реальный CLI вместо стаба 1.1 (изменён).
- `docs/cli.md` — спека компонента «поверхность gdau-logs» (новый).
- `tests/test_logs_api_cli.py` — offline-набор AC #1–#8 (новый).
- `tests/test_logs_api_cli_live.py` — live-smoke `evaluate` (opt-in) (новый).
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — статус истории 1-6 (изменён).

### Change Log

- 2026-05-24 — Реализованы CLI-примитивы жизненного цикла Logs API (`gdau-logs`): подкоманды
  `create`/`evaluate`/`status`/`download`/`clean`/`list`/`info`; поля из каталога (FR-2), clamp дат
  (1.4), креды от env-ридера (1.2→1.3), человекочитаемый вывод без `--format`. Заведена `docs/cli.md`.
  Добавлены offline-тесты (AC #1–#8) и opt-in live-smoke. Гейты зелёные (mypy strict, pytest 156),
  `uv.lock` не менялся. Status → review.

## Review Findings

_Code review 2026-05-24 (bmad-code-review, 3 слоя: Blind Hunter / Edge Case Hunter / Acceptance Auditor). Все 8 AC признаны выполненными; ниже — устойчивость и тестовое покрытие. Итог: 0 decision-needed, 5 patch, 1 defer, 9 dismissed._

- [x] [Review][Patch] `size` строкой/`null` в `parts` → `TypeError` мимо `except` в `main` (трейсбек, нарушение AC #4) [scripts/tools/logs_api_cli.py:247, 268] — `part.get("size", 0)` подставляет дефолт только при ОТСУТСТВИИ ключа; при `size` = строка (Yandex-API часто отдаёт числа строкой) или `null` арифметика `/(1024*1024)` (status) и `sum(...)` (list) бросает `TypeError`, который НЕ входит в `(ValueError, RuntimeError, FileExistsError, OSError)` → непойманный трейсбек. Путь `status`/`list` рендеринга размера НЕ верифицирован против реального API (офлайн-фикстуры все `int`; live-smoke бьёт только `evaluate`, без `parts`/`size`). Фикс: защитная коэрция `float(part.get("size") or 0)`. Источник: Edge Case Hunter (High).
- [x] [Review][Patch] `--clean`: сбой `clean` после успешного скачивания скрывает пути сохранённых файлов [scripts/tools/logs_api_cli.py:333-339] — `client.clean_log_request(...)` вызывается ДО `print` сводки сохранённых файлов; если очистка падает (`RuntimeError`), `main` ловит → exit 1, но оператор не узнаёт, КУДА легли уже успешно скачанные `.tsv`. Фикс: печатать сводку сохранённых файлов до попытки `clean`. Источник: Edge Case Hunter + Acceptance Auditor (Medium).
- [x] [Review][Patch] Распаковка `null`-значения ответа API → `AttributeError` мимо `except` [scripts/tools/logs_api_cli.py:195, 222, 347, 357] — `response.get("log_request", response)` (create/clean), `.get("log_request_evaluation", response)` (evaluate), `.get("counter", data)` (info) защищают только ОТСУТСТВИЕ ключа; при ключе со значением `null` вернётся `None` → `None.get(...)` = `AttributeError` (не в `except`-кортеже → трейсбек). `status`/`download` от этого защищены (`if not data`). Спекулятивно (успешный 200 с `null` маловероятен), но фикс дёшев и в духе fail-loud. Фикс: гард `isinstance(... , dict)` либо явная проверка. Источник: Edge Case Hunter (Low).
- [x] [Review][Patch] Подсказка после `create` без префикса `gdau-logs` [scripts/tools/logs_api_cli.py:200] — `print("Проверьте готовность командой: status --request-id <id>")`; реальный вызов — `gdau-logs status --request-id <id>`. Тривиальная правка текста. Источник: Acceptance Auditor (Low).
- [x] [Review][Patch] Ветка `_resolve_output` с суффиксом не покрыта тестом + размытое «→ файл» [scripts/tools/logs_api_cli.py:167; tests/test_logs_api_cli.py; docs/cli.md] — все download-тесты передают `--output` как каталог (или опускают); ветка `if path.suffix:` не исполняется ни разу (тестовая дисциплина — инвариант проекта). Поведение детерминировано (`--output foo.tsv` → `foo_part0.tsv`), но docstring/спека пишут «→ файл», что вводит в заблуждение (имя `foo.tsv` не создаётся; для N частей один файл невозможен). Фикс: добавить офлайн-тест на `--output <файл с суффиксом>` + уточнить формулировку (вывод всегда `{prefix}_partN.tsv`). Источник: Blind Hunter (High; занижено до Low с контекстом).
- [x] [Review][Defer] Частичная запись частей при сбое в середине цикла оставляет файлы на диске [scripts/tools/logs_api_cli.py:327-330] — deferred: атомарная запись (temp→rename под `.writer.lock`) — это Epic 2 (2.2/2.7), вне скоупа 1.6 (`download` = ad-hoc примитив по явному решению истории). AC #7 соблюдён (exit non-zero, успехом не выдаётся); остаётся лишь мусор частичных `.tsv`. Источник: Blind Hunter + Edge Case Hunter. → в `deferred-work.md`.

**Resolution (2026-05-24).** Все 5 patch применены (коэрция `size`, переупорядочение печати при `--clean`, гард null-распаковки ×4, префикс `gdau-logs` в подсказке, тест на ветку `_resolve_output` с суффиксом + уточнение docstring). Добавлены 2 регрессионных теста (строка/null `size`; ветка suffix). Гейты зелёные: `mypy scripts` чисто (11 файлов), `pytest` 158 passed (+2), 3 live deselected; `uv.lock` не менялся. 1 defer → `deferred-work.md`, 9 dismissed.
