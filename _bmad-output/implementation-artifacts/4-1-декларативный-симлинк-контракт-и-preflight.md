...
- [x] **Task 1 — `templates/paths-to-symlink.csv`: декларативный контракт (AC: #1, #2)**
  - [x] Создать файл с заголовком `path,comment` (форма directaiq `templates/paths-to-symlink.csv`). Колонка `path` — путь относительно корня (и dev-репо, и хранилища одинаков); `comment` — человекочитаемое назначение (может содержать запятые → RFC4180-парсинг обязателен).
  ...
  - [x] Файл коммитится (это контракт/шаблон, не данные). LF-окончания **обеспечены существующим `.gitattributes`** (`*.csv text eol=lf` + `* text=auto eol=lf`) — отдельных действий с `.gitattributes` не требуется (проверено).
  - [x] **Хвостовой перевод строки допустим:** LF-нормализация обычно добавляет финальный `\n`; `csv.DictReader` пустую хвостовую строку **не отдаёт** как запись (см. Task 2 — валидатор «пустой path» не должен спотыкаться о trailing newline; покрыть тестом, Task 4).
- [x] **Task 2 — `scripts/init/symlinks.py`: загрузка контракта + preflight + создание с откатом (AC: #1, #4–#9)**
  ...
  - [x] **Иерархия исключений (fail-loud):**
  ...
  - [x] **Чистый хелпер относительной цели (тестируемость, риск №2):** выделить `_relative_target(dev_repo_root: Path, storage_root: Path, rel: str) -> str` = `os.path.relpath(dev_repo_root / rel, start=(storage_root / rel).parent)`. Чистая строковая операция (ФС не трогает) → тест без способности создавать симлинки.
  - [x] **НЕ делать:** абсолютные цели (риск №2 — рвут переносимость); `rm -rf`/удаление реального не-симлинка (риск №4); `os.symlink` без `target_is_directory` на dir-целях (битый dir-симлинк на Windows); preflight через флаг `os.name` вместо реальной пробы (риск №3); резолюцию `GDAU_DATA_ROOT`/импорт `paths` (риск №1 — корни инъектируются); сырой `OSError`/`csv`-исключение наружу (обернуть); `str.split(",")` для CSV (риск №6); полный откат хранилища (шаблон/`.env`/БД — 4.3, риск №5); сетевые вызовы/открытие `gdau.duckdb`.
- [x] **Task 3 — Спека компонента `docs/init-and-storage.md` (новая, часть DoD)**
  - [x] Завести `docs/init-and-storage.md` (project-context.md:64 — логический компонент «init + симлинки + два-репо»; 4.1 первая → создаёт файл, 4.2/4.3 дополнят). Человеческим языком, без жаргона/сигнатур. Раздел **«симлинк-контракт и preflight»** на три вопроса: **(1) Что делает** — при разворачивании игры связывает её папку с инструментами юнита **по явному списку** (`paths-to-symlink.csv`): код, каталог схемы, справочники, конфиги — это **ссылки** на dev-репо, а не копии; перед связыванием **проверяет**, что система вообще умеет создавать такие ссылки, и если нет — **сразу останавливается с понятной инструкцией**, ничего не разворачивая наполовину; **(2) Зачем** — чтобы исправление/обновление инструмента в одном месте (dev-репо) **сразу видели все игры** без копирования и рассинхрона; чтобы папку игры можно было **перенести на другую машину/ОС** копированием — ссылки заданы **относительно** и не рвутся; **(3) Контракт** — список ссылок — единый источник (правится в одном файле, не разбросан по коду); существующая ссылка при повторном развороте **обновляется** (не падает), а реальный файл/каталог на месте ссылки **останавливает** разворот (данные не удаляются); если в списке указана цель, которой нет в dev-репо, — **понятная ошибка**; если связать удалось частично и что-то сорвалось — уже созданные ссылки **снимаются** (нет полусвязанного состояния). **Границы:** копирование шаблона хранилища и файл описания игры — 4.2; полный разворот `gdau-init` (имя → шаблон → ссылки → `.env` → окружение → БД → git) и откат всего хранилища — 4.3; данные и `.env` живут в хранилище, инструмент — в dev-репо.
- [x] **Task 4 — Offline-тесты `tests/test_init_symlinks.py` (AC: #1, #4–#9; решение D7)**
  - [x] `from __future__ import annotations`; зеркалит `scripts/init/symlinks.py` → `tests/test_init_symlinks.py` (имя из architecture.md:485). Кросс-платформенно (`tmp_path`/`pathlib`); CI ubuntu + windows.
  - [x] **Чистые тесты контракта (AC #1, #8 — всегда идут):** валидный CSV (`tmp_path`-фикстура) → ожидаемый `list[str]` в порядке файла; `comment` с запятой (закавыченной) не рвёт парсинг (RFC4180); **хвостовая пустая строка** (файл оканчивается `\n` — типовой LF-случай) парсится **без** ошибки (DictReader её не отдаёт); **лишняя незакавыченная колонка** (строка с двумя запятыми) → `SymlinkContractError` (restkey-страж); пустой контракт → `SymlinkContractError`; дубли `path` → `SymlinkContractError`; битый заголовок (напр. `path,note`) → `SymlinkContractError`; отсутствие файла / битый симлинк-путь → `SymlinkContractError` с путём; пустой/пробельный `path` → ошибка.
  - [x] **Чистый тест относительной цели (AC #6):** `_relative_target(dev_repo_root, storage_root, "scripts")` при `storage_root = dev_repo_root.parent / "game1"` → `os.path.join("..", "gamedev-analytics-unit", "scripts")` (или эквивалент через `Path`); утверждать, что результат **не абсолютен** (`not os.path.isabs(...)`). Без создания симлинков.
  - [x] **Ветка provala preflight (AC #4 — детерминированно, любая ОС):** `monkeypatch.setattr(os, "symlink", <raises OSError>)` → `preflight_symlink_capability(probe_dir=tmp_path)` → `pytest.raises(SymlinkPreflightError)`; сообщение содержит «Developer Mode» (или «Режим разработчика»); после вызова в `tmp_path` не осталось пробных файлов (проба убрана в `finally`).
  - [x] **Детекция отсутствующей цели (AC #5):** контракт с `path`, которого нет в `dev_repo_root` → `create_symlinks(...)` → `pytest.raises(SymlinkTargetMissingError)`; в `storage_root` **не создано ни одного** симлинка (предвалидация до создания).
  - [x] **Capability-gated фикстура:** фикстура зовёт `preflight_symlink_capability()`; `SymlinkPreflightError` → `pytest.skip("нет способности создавать симлинки (Windows без Developer Mode)")`. Все тесты ниже — под ней.
  - [x] **Реальное создание + относительность (AC #2, #6, gated):** `dev_repo_root` с реальными `scripts/`(каталог)+`pyproject.toml`(файл) на `tmp_path`; `storage_root = tmp_path/"game"`; `create_symlinks(dev_repo_root=…, storage_root=…, contract_path=<фикстура>)` → для каждой записи `(storage_root/rel).is_symlink()` True; **проверять относительность как `not os.path.isabs(os.readlink(link))`** (НЕ сравнение строк — `\`/`/` и Windows-префиксы разъедут; `os.readlink` для относительной цели возвращает относительный путь — проверено); переход по ссылке резолвится в цель dev-репо; чтение содержимого через линк отдаёт файл dev-репо (AC #3 — правка в dev-репо видна через ссылку).
  - [x] **AC #3 — осознанно без always-run теста:** единственная проверка AC #3 (read-through) — под capability-фикстурой (на Windows-CI без Dev Mode скипается). Реальное покрытие даёт ubuntu-прогон (симлинки нативны). Это by-design (как defer 1.5) — зафиксировать в Dev Agent Record, чтобы Acceptance Auditor не счёл дырой.
  - [x] **Идемпотентность + замена существующего симлинка (AC #7, gated):** повторный `create_symlinks(...)` на готовом наборе **не падает** (`FileExistsError` не летит), ссылки на месте; предварительно подложить симлинк с «неправильной» целью → после вызова он указывает на правильную (замена).
  - [x] **Реальный не-симлинк по пути → fail-loud (AC #7-fail, gated):** в `storage_root` создать реальный файл по адресу будущего линка → `create_symlinks(...)` → `pytest.raises(SymlinkError)`; реальный файл **не удалён** (риск №4).
  ...
  - [x] **Анти-зависимость (через `ast`, по import-узлам):** в `scripts/init/symlinks.py` нет top-level импорта `pandas`/`polars`/`numpy`/`pyarrow`, directaiq-инфры `config_manager`/`base_script`, и нет `duckdb`/`scripts.utils.paths`/`scripts.utils.database_manager` (риск №1 — модуль независим, знает только ФС + контракт). Приём — `tests/test_parquet_store.py` (ast по import-узлам, не подстрока).
  - [x] **Live-тест НЕ нужен** (и не заводить): 4.1 — ФС-операции, без внешнего API. Зафиксировать в Dev Agent Record (как 2.1–2.6), чтобы отсутствие live не сочли упущением.
- [x] **Task 5 — Гейты верификации (обязательны перед закрытием)**
  - [x] `uv run mypy scripts` → зелено (strict; `list[str]`/`list[Path]`; `os.symlink` с `target_is_directory`; без `Any`-дыр). Прогнать **win32 + `--platform linux`** (как 2.5/2.6/2.7 — кросс-OS strict).
  - [x] `uv run pytest` → зелено (новый offline-набор + регрессия 1.x/2.x; live отсеян `addopts="-m 'not live'"`). Реальные симлинк-тесты идут или скипаются (Dev Mode) — **в логе убедиться, что не упали**; на ubuntu CI они выполняются (там симлинки нативны) — реальное покрытие AC #2/#6/#7/#9 обеспечивает Linux-прогон.
  - [x] Новых зависимостей нет (`csv`/`os`/`tempfile`/`contextlib`/`pathlib`/`logging` — stdlib) → **`uv.lock` не меняется**.
  - [x] `data/`-артефактов (`*.parquet`/`*.duckdb`/`.writer.lock`) в dev-репо не создано (4.1 их вообще не трогает). `templates/paths-to-symlink.csv` — новый коммитируемый файл.
...
- `uv run pytest tests/test_init_symlinks.py -v` → 18 passed (capability-gated реальные симлинки выполнены — на dev-машине Windows Developer Mode включён).
- `uv run mypy scripts` (win32) → Success: no issues found in 20 source files.
- `uv run mypy scripts --platform linux` → Success: no issues found in 20 source files.
- `uv run pytest` → 351 passed, 8 deselected (live) — регрессия 1.x/2.x зелёная, ноль ломок.
- `git diff --stat uv.lock` → пусто (лок не менялся; всё stdlib). `git status` — нет `*.parquet`/`*.duckdb`/`.writer.lock` (4.1 их не трогает).

### Completion Notes List

Реализованы три ФС-операции в новом модуле `scripts/init/symlinks.py` (зеркало паттернов `catalog.py`/`test_parquet_store.py`); потребитель — `init_project.py` 4.3 (остался стабом, не тронут). Решения D1–D7 из контекста истории реализованы дословно:

- **D1** — модуль в `scripts/init/` (не `utils/`), только stdlib (`csv`/`os`/`shutil`/`tempfile`/`contextlib`/`pathlib`/`logging`); корни `dev_repo_root`/`storage_root` инъектируются. Анти-зависимость закреплена ast-тестом (нет `duckdb`/`paths`/`database_manager`/тяжёлого стека).
- **D2** — цели **относительные** через `_relative_target` (`os.path.relpath` от родителя линка) — чистая строковая операция, тестируется без способности создавать симлинки; относительность в gated-тесте проверяется `not os.path.isabs(os.readlink(link))` (У1 ревью, не сравнением строк).
- **D3** — `preflight_symlink_capability` = реальная проба `os.symlink` в temp + fail-loud `SymlinkPreflightError` с инструкцией Developer Mode; проба убирается в `finally` (`shutil.rmtree(ignore_errors=True)` для своего temp — У6 ревью, без протечки). Ветка provala покрыта `monkeypatch`-ом детерминированно на любой ОС.
- **D4** — существующий **симлинк** → идемпотентная замена (`os.unlink`+создать); существующий **реальный** файл/каталог → fail-loud `SymlinkError` (НЕ `rm -rf` directaiq); отсутствует → создать.
- **D5** — shipped-CSV = 4 существующие стабильные цели (`scripts`/`development-docs`/`yandex-docs`/`pyproject.toml`); `.mcp.json`(нет до Epic 3)/`.claude/*` отложены до 4.3. Отдельный тест `test_shipped_contract_loads_and_targets_exist` — страж: реальный контракт грузится и все цели существуют (иначе 4.3 упал бы `SymlinkTargetMissingError`).
- **D6** — откат (`except BaseException` → `reversed(created)` под `suppress(OSError)`) снимает только симлинки **этого вызова**; полный откат хранилища — 4.3, здесь не дублируется.
- **D7** — тесты: чистая логика (валидация контракта + relpath) всегда идёт; реальные симлинки под capability-фикстурой (`pytest.skip` без Dev Mode — GH windows-runner не валит); ветка provala preflight через `monkeypatch`. Закрыт defer 1.5 (битый симлинк/Dev Mode флак).

**Ревью-правки применены** (из Change Log create-story): **К1** — тест отката AC #9 зовёт `create_symlinks(run_preflight=False)` (иначе проба preflight съела бы первый расход monkeypatch-счётчика). Дополнительно: тест AC #5 (missing target) тоже передаёт `run_preflight=False` — преднамеренно, т.к. проверяет именно предвалидацию целей и должен быть детерминированным независимо от способности платформы (preflight в `create_symlinks` идёт ПЕРВЫМ, на Windows-no-DevMode иначе поднял бы `SymlinkPreflightError` вместо `SymlinkTargetMissingError`). **У3** — хвостовая пустая строка LF-файла не валит валидатор (тест `test_trailing_blank_line_is_not_a_record`). **О1** — `csv.DictReader(restkey=_RESTKEY)` как `catalog.py` (лишняя незакавыченная колонка → fail-loud).

**AC #3 — осознанно без always-run теста** (зафиксировано, как U5 ревью и паттерн defer 1.5): единственная проверка read-through (`marker.txt` сквозь симлинк) — под capability-фикстурой; на Windows-CI без Dev Mode скипается, реальное покрытие даёт ubuntu-прогон (симлинки нативны). Это by-design, не дыра — Acceptance Auditor.

**Live-набор осознанно отсутствует** (как 2.1–2.6): 4.1 — чисто ФС-операции, в сеть не ходит, БД не открывает; правило opt-in live применимо только к внешнему Logs API ([[realapi-smoke-tests]]).

`docs/init-and-storage.md` заведён (новая спека компонента «init + симлинки + два-репо», первая init-история создаёт файл — 4.2/4.3 дополнят): три вопроса простыми словами + границы 4.2/4.3 + «обновление в dev-репо видно всем играм» + «перенос не рвёт ссылки».

### File List

- `templates/paths-to-symlink.csv` — **новый**: декларативный симлинк-контракт (форма directaiq `path,comment`, 4 стабильные цели; LF, utf-8).
- `scripts/init/symlinks.py` — **новый**: `load_symlink_contract`/`preflight_symlink_capability`/`create_symlinks` + чистый `_relative_target` + иерархия исключений `SymlinkContractError`/`SymlinkTargetMissingError`/`SymlinkPreflightError`/`SymlinkError`.
- `docs/init-and-storage.md` — **новый**: человекочитаемая спека компонента init/симлинки/два-репо.
- `tests/test_init_symlinks.py` — **новый**: 18 offline-тестов (чистая логика + capability-gated реальные симлинки + ветка provala preflight + ast-анти-зависимость).
- `_bmad-output/implementation-artifacts/sprint-status.yaml` — статус 4-1 `ready-for-dev` → `in-progress` → `review`.

## Change Log

- 2026-05-25 — **dev-story: реализация 4.1 (статус → review).** Все 5 задач закрыты, 9 AC покрыты. Артефакты: `templates/paths-to-symlink.csv` (новый, 4 стабильные цели — D5), `scripts/init/symlinks.py` (новый: контракт RFC4180 как `catalog.py` / preflight реальной пробой + fail-loud Dev Mode / относительные цели `os.path.relpath` + предвалидация + откат частичного набора), `docs/init-and-storage.md` (новая спека), `tests/test_init_symlinks.py` (18 тестов, D7: чистая логика + capability-gated + monkeypatch-провал preflight + ast-анти-зависимость). Решения D1–D7 реализованы дословно; ревью-правки К1/У1/У3/У5/У6/О1 применены; AC #3 и отсутствие live осознанно зафиксированы (by-design). Гейты зелёные: `mypy strict` win32+linux 20 файлов, `pytest` 351 passed (+18) / 8 live deselected, `uv.lock` не менялся, data-артефактов в dev-репо нет. `init_project.py` остался стабом (потребитель — 4.3).
...
_Code review (3 слоя: Blind Hunter / Edge Case Hunter / Acceptance Auditor, Opus), 2026-05-25. Acceptance Auditor (полный контекст): все 9 AC = PASS, DoD-чеклист (12 пунктов) выполнен, нарушений спеки/решений D1–D7/инвариантов не найдено. Итог триажа: 2 patch, 0 decision-needed, 0 defer, 13 групп находок отклонено (ложные/by-design/вне скоупа). Два «высокой серьёзности» утверждения слепых слоёв проверены эмпирически на Windows-машине с Dev Mode._

### Patch (требуют правки)

- [x] [Review][Patch] Cross-drive `os.path.relpath` бросает сырой `ValueError` мимо доменной иерархии [scripts/init/symlinks.py:271] — если `dev_repo_root` и `storage_root` на разных дисках/точках монтирования Windows (напр. dev-репо `G:\`, хранилище `C:\`), `_relative_target` бросает `ValueError: path is on mount 'C:', start on mount 'G:'` (эмпирически подтверждено). Вызов внутри `try/except BaseException` → откат своих симлинков отрабатывает, но наружу улетает СЫРОЙ `ValueError` с криптической формулировкой, не обёрнутый в `SymlinkError` (нарушает инвариант «никогда сырое stdlib-исключение наружу»). Относительный симлинк между дисками невозможен в принципе → падать правильно, но fail-loud доменной ошибкой с инструкцией «размести хранилище на том же диске, что dev-репо (требование относительных симлинков, NFR-2)». Источник: blind+edge.
- [x] [Review][Patch] Нет валидации, что `path` в контракте относителен и без `..` [scripts/init/symlinks.py:149] — `load_symlink_contract` принимает `path` как есть; абсолютный (`/etc`, `C:\Windows`) или с `..` (`../../x`) молча уйдёт за пределы: `dev_repo_root / rel` при абсолютном `rel` отбрасывает корень (поведение `Path.__truediv__`), предвалидация `target.exists()` проверит уже не ту цель. Контракт — доверенный SSOT-артефакт (не пользовательский ввод), но 4.3 будет ДОПИСЫВАТЬ в него вложенные записи (`.claude/...`) вручную → дешёвый fail-loud-гард (отклонять `os.path.isabs(value)` и `'..' in Path(value).parts`) ловит опечатку рано и согласуется с «в спорной — строже». Опциональное упрочнение. Источник: blind+edge.

### Отклонено (кратко, для протокола)

- [x] [Review][Dismiss] `os.unlink` vs `os.rmdir` для dir-симлинка на Windows (Blind/Edge) — ЛОЖНО: эмпирически `os.unlink` штатно снимает directory-симлинк (Dev Mode вкл.); подтверждено и зелёным тестом отката на трёх dir-симлинках.
- [x] [Review][Dismiss] BOM в заголовке CSV ломает валидацию (Edge) — осознанное зеркало `catalog.py` (`encoding="utf-8"`), shipped-файл чистый LF без BOM; смена только здесь разошлась бы с проектным паттерном.
- [x] [Review][Dismiss] Откат не снимает промежуточные `mkdir`-каталоги (Blind/Edge) — by-design (спека, риск №5: пустой каталог безвреден); для shipped-CSV неактуально (все записи верхнего уровня).
- [x] [Review][Dismiss] `except BaseException` ловит `KeyboardInterrupt` (Blind) — дословно по спеке (Task 2), идёт `raise` → семантика прерывания сохранена, усиливает AC #9.
- [x] [Review][Dismiss] Фикс-имена пробы preflight, коллизия в shared `probe_dir` (Blind/Edge) — прод использует уникальный `tempfile.mkdtemp`; инъекция `probe_dir` — только тесты на изолированном `tmp_path`.
- [x] [Review][Dismiss] Любая `OSError` в preflight → совет про Dev Mode (Blind) — реальная причина приложена в сообщении (`Причина: {exc}`); основной кейс Windows — именно привилегия.
- [x] [Review][Dismiss] Прочие мелочи (Blind/Edge): дедуп без casefold, короткая строка без comment, дрейф номера строки при mid-blank, `target.exists()` следует за симлинками, two-loop TOCTOU, нет общего базового исключения, ast только по import-узлам, preflight абсолютная vs относительная цель — по доверенному артефакту/модели «один оператор» или by-design по спеке; на shipped-составе не срабатывают.
