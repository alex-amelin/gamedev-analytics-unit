...
- **CI (AC #6) — ПОДТВЕРЖДЕНО:** локально на Windows воспроизведены все три шага CI (`uv sync --frozen`, `uv run mypy scripts`, `uv run pytest`) — все exit 0; матрица `[ubuntu-latest, windows-latest]` с `fail-fast: false` прописана. После push на `main` (коммит `2a932c7`) GitHub Actions прогнал матрицу **зелёным на обеих ОС** (подтверждено Шефом 2026-05-23) — AC #6 закрыт эмпирически.
