...
_Code review 2026-05-24 (bmad-code-review; три слоя: Blind Hunter / Edge Case Hunter / Acceptance Auditor). Итог: 1 decision-needed → resolved (вариант A), 3 patch, 0 defer, 10 отброшено как шум. Acceptance Auditor: все 6 AC закрыты, границы «НЕ делать» и инварианты project-context соблюдены, блокеров приёмки нет._

- [x] [Review][Patch] `reconcile` падает до создания таблицы — если `reconcile` вызвать до `ensure_load_state_table`, `SELECT … FROM load_state` в `_load_meta` бросает `CatalogException` (подтверждено: подкласс `duckdb.Error`, но `_load_meta` его не ловит) → весь проход реконсиляции падает исключением, что противоречит духу AC #5 «один сбой не валит всё обновление». **Решение Шефа (2026-05-24): вариант A — `reconcile` зовёт `ensure_load_state_table(conn)` в начале (идемпотентно; строже — в духе project-context «в спорной — строже вокруг целостности БД»).** [Edge: high, Blind: medium] [scripts/utils/load_state.py:326,360-362]
- [x] [Review][Patch] `count_partition_rows`: `path.is_file()` вместо `path.exists()` + ловить `OSError` — директория с именем `{date}.parquet` проходит `.exists()==True`, и `read_parquet` глобит вложенные parquet, возвращая агрегированный count (проверено: count=3) → ложный факт может ложно подтвердить день «загружен»; `is_file()` это закрывает. Плюс `path.exists()`/`read_parquet` при сбое ФС (битый mount/права) могут бросить `OSError` мимо текущего `except duckdb.Error` → уронить весь проход (подрывает AC #5). [Edge: high + Blind: medium] [scripts/utils/load_state.py:291-303]
- [x] [Review][Patch] Тест двухфазной отметки не покрыт — `mark_loading`/`mark_failed` тестируются только на пустой таблице; сценарий «`mark_loaded` → затем `mark_loading`/`mark_failed` обнуляет `row_count`/`loaded_at`» (суть crash-recovery AC #6 — двухфазная отметка p81) не проверен. Добавить тест. [Blind: high] [tests/test_load_state.py]
