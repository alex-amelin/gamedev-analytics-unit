...
- [ ] **Task 1 — `scripts/utils/load_state.py`: таблица + отметки + реконсиляция (AC: #1–#6)**
  ...
  - [ ] **`ensure_load_state_table(conn: duckdb.DuckDBPyConnection) -> None` (AC #1):**
  ...
  - [ ] **Отметки чекпойнта (точка коммита дня; AC #6):**
  ...
  - [ ] **Счёт строк факта партиции (AC #5, риск №3):**
  ...
  - [ ] **Реконсиляция мета×факт (AC #2, #3, #4, #5, #6):**
